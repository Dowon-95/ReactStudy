...
async function commToServer() {
  // fetch API 사용
  // 서버와 통신시, 서버에서 언제 응답 (요청한 결과, response) 이 빠를수도 있고 느릴수도 있음
  // => 비동기 처리
...
function display(users) {
  ...
  arr.forEach((element) => {
    ...
    let user = document.createTextNode(element.id);
    newP.appendChild(user);
