...
var age2 = 29; // 일반적으로는 let키워드로 변수 선언해서 사용.
...
/* 변수 이름 작성 
    - 카멜 표기법
      let totalSaleCount = 10;
    - 파스칼 표기법
      let TotalSalesCount = 10;
    - 스네이크 표기법
      let total_scale_count = 10;
*/
...
//Infinity
// 다른 언어와 달리 0으로 나누어도 오류가 발생하지 않고, infinity 가 결과로 반환됨.
console.log(1 / 0);

// NaN : Not a Number
// 수학적으로 연산이 불가능하면, NaN이 반환됨.
...
console.log(result); //1020  => 10을 문자로 형변환 후 문자열 합치기. : 묵시적 형변환
...
let numAA = parseInt(strA, 10); // strA 를 10진수 숫자로
let numBB = parseInt(strB, 10); // strB 를 10진수 숫자로
...
console.log(strvarB); //문자열 null 가 반환
console.log(strvarC); //문자열 true 가 반환

// boolean 으로 형변환 : 문자열은 true, 숫자 1  true , 숫자 0 flase, blank false.
let varTrue = "1"; //true
...
console.log(varA ?? varB); //10 => 연산자의 왼족을 기준.
...
if (num === 10) {
  console.log("조건일치");
  console.log("num 10입니다.");
} else if (num > 10) {
  console.log("조건 일치!");
  console.log("num은 10보다 큽니다");
} else {
  console.log("조건 불일치");
  console.log("num으 10보다 작습니다");
}
 ...
 * 마치 호출보다 먼저 함수를 선언한 것처러 자연스럽게 동작하고 있음.
 *
