 ...
 *
 * 2. 조건부에서 사용될 수 없다. 조건문이나 반복문 내부에서 호출할수 없음 => 오류발생
 *
 * 3. custom hook 을 직접만들어서 사용 가능
 *    작성 방법은 함수 작성시 함수명 앞에 접두어 use 를 붙이면 됨.
 *
 *    좋은 활용 법
 *    custom hook 을 통해 컴포넌트 내부에서 만족되는 부분을 hook 을 통해 반복 사용할 수 있도록 분리할수 있음
 *
 *    그리고, custom hook 은 별도의 디렉토리에 hooks 이름의 폴더를 만들어서 관리하는 것이 일반적임.
 */

// 1.
// react hook 을 함수 컴포넌트 외부에 사용 불가
// 브라우저에서도 오류가 발생.
...
const HookExam = () => {
   ...
   *
   * 하지만 여러개의 input tag 를 반복해서 사용하는 경우라면.
   *
